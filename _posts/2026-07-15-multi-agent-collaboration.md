...
```python
# Round 1：中心节点拥有完整目标，拆出彼此独立的 TaskSpec。
tasks = supervisor.plan(request)
# 隐藏状态：只有 Supervisor 保存全部 results；Worker 之间互不可见。
results = parallel_run(workers, tasks)
missing = required_fields - merged_fields(results)

# Round 2：若缺字段，中心节点把“缺口”重新派给最合适的 Worker。
while missing:
    repair_task = {"missing": missing, "known_results": results}
    results.update(supervisor.reassign(repair_task))
    missing = required_fields - merged_fields(results)
...
>>> supervisor.debug_hidden_state()
{"round": 1, "results": {}, "missing": ["venue", "deliverables", "budget"]}
>>> supervisor.collect({"Research": {"venue": "城市美术馆"}, "Budget": {"budget": 7800}})
{"round": 1, "missing": ["deliverables"], "next_agent": "ContentWorker"}
>>> ContentWorker.repair({"missing": ["deliverables"]})  # Round 2
{"deliverables": ["主视觉", "路线手册"], "next_agent": "Supervisor"}
...
```python
transcript = [request]
# transcript 是隐藏的共享状态；每轮 append 都改变下一位 Agent 的上下文。
for round_no in range(max_rounds):
    speaker = select_next_role(transcript)
    message = speaker.respond(transcript)
    transcript.append(message)
    # 主持人读取最新 transcript，决定继续讨论、要求修订或结束。
    decision = facilitator.review(transcript)
    if decision == "revise":
        transcript.append(facilitator.request_revision())
...
(2, "ResearchAgent")
>>> Facilitator.review(transcript)  # Round 1：主题还没有预算约束
"revise"
>>> transcript[-1]
{"sender": "Facilitator", "content": "请 BudgetAgent 补充资源边界"}
>>> BudgetAgent.speak(transcript)    # Round 2：读取前面全部三条消息
"预算 7800 元，可执行"
...
```python
# state 是跨 Agent 隐藏累积的工作记忆；current 是唯一控制权。
state, current, hop = initialize(request), ResearchAgent, 0
while current:
    hop += 1
    envelope = {"state": state.copy(), "from": current.name, "hop": hop}
    patch, next_agent = current.handle(envelope)
    # 每一棒只写自己的 patch，运行器负责合并，避免覆盖前人结果。
    state = merge(state, patch)
    save_handoff_checkpoint(state, current, next_agent, hop)
    current = next_agent              # 控制权与最新 state 一起交接
return state
```
...
>>> runtime.debug_hidden_state()
{"hop": 1, "owner": "ContentAgent", "state": {"venue": "城市美术馆与历史街区"}}
>>> ContentAgent.handle(runtime.next_envelope())  # Round 2
({"deliverables": ["主视觉", "路线手册"]}, "BudgetAgent")
>>> BudgetAgent.handle(runtime.next_envelope())   # Round 3
({"budget": 7800}, None)
```
...
```python
# State 是唯一跨节点共享的隐藏状态；节点只返回增量 patch。
state, node = initial_state(request), "research"
while node != "END":
    snapshot = state.copy()            # 节点只能读到已提交的版本
    patch = graph[node].run(snapshot)
    state = reducer(state, patch)      # 原子提交，并递增 checkpoint_version
    node = route_by_condition(node, state)
    # 预算超限时，条件边会回到 content 节点，形成显式的第二轮。
return state
```
...
>>> BudgetNode.route({"budget": 8600, "budget_limit": 8000})  # Round 1 未通过
"ContentNode"
>>> runner.debug_hidden_state()
{"checkpoint": 4, "route": "ContentNode", "reason": "budget_exceeded"}
...
```python
# Bus 的 queue 与订阅表是隐藏运行状态；发布者不知道具体消费者。
bus.publish(Event("event.created", request))
while event := bus.pop():
    for agent in bus.subscribers(event.topic):
        patch, followups = agent.handle(event.payload, state.copy())
        state.apply(patch)             # 写入共享状态后才发布下一轮事件
        bus.publish_many(followups)

# CompletionChecker 读取 state，而不是把“队列空”当作完成。
...
>>> bus.debug_hidden_state()
{"queue": ["venue.ready", "content.ready"], "state": {"venue": "城市美术馆", "budget": 7800}}
>>> ContentAgent.on_event({"topic": "venue.ready"})  # Round 2：消费 Research 的后续事件
{"patch": {"deliverables": ["主视觉", "路线手册"]}}
