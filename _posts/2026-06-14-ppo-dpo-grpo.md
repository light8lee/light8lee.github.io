...
RLHF 要解决如下优化问题：
![Pasted image 20260505122330]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505122330.png' | relative_url }})


- 第一项鼓励高奖励，第二项惩罚策略偏离 SFT 模型 πref，防止语言模型退化。
...
![Pasted image 20260505122330]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505122330.png' | relative_url }})
![Pasted image 20260505122349]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505122349.png' | relative_url }})
其中 Z(x) 是配分函数（归一化因子）。  
...
- **Policy Model** πθ​：正在优化的策略
    
- **Old Policy** πθold​​：采样用的冻结策略
    
- **Reference Model** πref：SFT 模型，用于计算 KL 惩罚
    
- **Critic (Value Model)** Vϕ：估计状态价值，用于计算优势
...
# 2. DPO
![Pasted image 20260505122710]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505122710.png' | relative_url }})
...
![Pasted image 20260505122914]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505122914.png' | relative_url }})
...
- 优势函数 AtAt​ 依赖于总回报 RtRt​，而 RtRt​ 需要从序列末尾反向累积。
...
3. **回溯阶段**：从最后一个 token 开始，反向计算每一步的回报 RtRt​ 和优势 AtAt​。
...
是的。GRPO 也需要对每个 prompt 一次采样 G 个完整回答，得到终局奖励后，再做组内标准化得到序列级优势 Ai​，再让这个回答的所有 token 共享该优势。  
...
| -------------- | ---------------------- | ---------------------------------------- |
| **评估对象**       | **完整回答** y             | **生成过程中的状态** st​（前 t 个 token）            |
| **输出含义**       | 整个回答的质量（是否符合人类偏好）      | 从状态 st​ 开始，未来能获得的**预期总回报**               |
| **粒度**         | **序列级**（一个回答一个分数）      | **Token 级**（每个 token 位置都输出一个值）           |
| **训练阶段**       | RLHF 第 2 步，**独立训练并冻结** | PPO 阶段**在线训练，不断更新**                      |
| **训练信号**       | 人类偏好对（胜者 vs 败者）        | 实际产生的总奖励 RtotalRtotal​（含奖励模型给出的分和 KL 惩罚） |
| **是否参与优势计算**   | 不直接参与，只是提供 r(x,y)      | **直接参与**，作为基线来算优势：$A^t=Rt−V(st)$         |
...
![Pasted image 20260505123639]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505123639.png' | relative_url }})
而 **Value Model 的训练目标** 就是去近似这个 RtotalRtotal​ 的期望：

![Pasted image 20260505123659]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505123659.png' | relative_url }})
...
- **保留 Reward Model（或规则）** → 仍然需要为每个回答打一个最终分 riri​。
...
![Pasted image 20260505123829]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505123829.png' | relative_url }})
...
Value Model （Critic）的输出是**状态价值函数** Vϕ(st)Vϕ​(st​)，它预测的是“从当前状态 stst​ 出发，按照当前策略继续生成，未来能拿到的**期望总奖励**（含 KL 惩罚）”。
...
每一步都可以用 $r_{next}+V(s_{t+1})−V(s_t)$ 来构造密度更高的**时序差分（TD）奖励**，彻底解决稀疏问题。
...
![Pasted image 20260505124023]({{ '/assets/posts/llm-notes/ppo-dpo-grpo/images/pasted-image-20260505124023.png' | relative_url }})
...
- 因为奖励仍然是序列级稀疏的，GRPO 会让一个回答的**所有 token 共享同一个优势** AiAi​，这在结果导向非常强的任务（数学、代码的对错）中效果很好。
